...
| 指标 | 目标 | 实测 | 达标 |
|---|---:|---:|---|
| 上传接口耗时（在线，经代理） | 快速返回 task_id | 15,136ms（跨区域多跳延迟为主） | ⚠️ 同区域可达 P95≤1s |
| 消费触发 | - | HTTP 200，6/6 批次 | ✅ |
| 全链路总耗时（上传→完成） | ≤ 60,000ms | **10,385ms** | ✅ |
...
### 3.3 多用户并发访问压测（2026-08-06，3 并发用户）
...
| 墙钟总耗时 | - | 33,877ms | - |
| 平均上传耗时 | - | 16,678ms（跨区域） | - |
| 平均全链路耗时 | - | 32,466ms | ✅（<60s） |
| HTTP 5xx | 0 | 0 | ✅ |
| 每用户结果 | 成功行入库 | PARTIAL_SUCCESS，成功 9,950 / 失败 50 | ✅ |

### 3.4 本地直连压测（2026-08-05，本机直连 Neon）
