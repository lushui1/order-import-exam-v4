...
| 模式 | 落地 |
|---|---|
| 异步任务表 | `import_tasks`：status/total_rows/processed_rows/success_rows/failed_rows/total_batches/completed_batches/trace_id/degraded；`file_data`(bytea) 存文件内容解决 Vercel Serverless /tmp 跨实例不共享（PRD 3.1） |
